...
import { Card, Button, LoadingSpinner, EmptyState, Table, TableHeader, TableBody, TableRow, TableHead, TableCell, Badge, PageHeader } from '../../components/ui';
...
export default function ServicesDemandesEngagements() {
  ...
  return (
    <div className="space-y-6">
      {/* Header premium */}
      <PageHeader
        title="Demandes d'Engagements"
        subtitle="Créer et suivre vos demandes d'engagements"
        kpis={[
          {
            label: 'Demandes',
            value: filteredEngagements.length,
            sub: 'tous statuts',
            icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>,
          },
          {
            label: 'En brouillon',
            value: filteredEngagements.filter(e => e.statut === 'brouillon').length,
            sub: 'à soumettre',
          ...
          {
            label: 'Approuvées',
            value: filteredEngagements.filter(e => e.statut === 'approuve').length,
            sub: 'par DG',
            icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>,
          },
          {
            label: 'Montant total',
            value: formatMontant(filteredEngagements.reduce((s, e) => s + (parseFloat(e.montant) || 0), 0)),
            sub: 'demandé',
